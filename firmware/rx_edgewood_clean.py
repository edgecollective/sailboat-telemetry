...
while True:
    ...
    if len(msg) > 0:
        ...
        try:
            ...
            if len(parts) == 4:
                sailboat_id = parts[0].strip()
                
                # Validate sailboat_id contains only alphanumeric characters
                if not sailboat_id.replace('_', '').replace('-', '').isalnum():
                    continue
                ...
                # Output format: sailboat_id,lat,lon,tilt
                print(f"{sailboat_id},{lat},{lon},{tilt}")
            
        except (ValueError, UnicodeDecodeError):
            # Discard any input that can't be parsed as expected numeric format
            pass
